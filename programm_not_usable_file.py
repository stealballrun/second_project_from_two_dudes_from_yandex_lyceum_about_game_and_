...
for i in range(len(field)):
    for j in range(len(field[i])):
        ...
        cell3 = three[col][row]
        cell4 = four[col][row]
        if cell1[0] == -1 or cell1[1] == 11 or cell1[1] == -1 or cell1[0] == len(field[col]):
            one[col][row] = (-2,0)
        if cell2[0] == -1 or cell2[1] == 11 or cell2[1] == -1 or cell2[0] == len(field[col]):
            two[col][row] = (-2,0)
        if cell3[0] == -1 or cell3[1] == 11 or cell3[1] == -1 or cell3[0] == len(field[col]):
            three[col][row] = (-2,0)     
        if cell4[0] == -1 or cell4[1] == 11 or cell4[1] == -1 or cell4[0] == len(field[col]):
            four[col][row] = (-2,0)            
